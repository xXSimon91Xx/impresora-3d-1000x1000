# El equipo

<p align="center">
  <img src="assets/logo-institut-jaume-huguet.png" alt="Institut Jaume Huguet — Antiga Escola del Treball, Valls" width="400"/>
</p>

> **Cicle Formatiu de Fabricació Additiva (CEFP) — Curso 2025-2026**  
> Proyecto realizado en el **Institut Jaume Huguet — Antiga Escola del Treball**, Valls.  
> Centro con más de 100 años de historia formando a profesionales técnicos en el Camp de Tarragona.  
> 🌐 [institutjaumehuguet.cat](https://institutjaumehuguet.cat)

---

## Agradecimientos

Un agradecimiento especial al **director del instituto** por confiar en el proyecto, aportar el presupuesto necesario para los componentes y ceder el espacio del taller para la construcción. Sin su apoyo este proyecto no habría sido posible.

---

## Profesores

| Nombre | Rol |
|--------|-----|
| **Fèlix Garcia Garcia** | Tutor técnico del proyecto. Supervisión general, revisión de documentación y validación técnica. |
| **Héctor Zazo Jiménez** | Apoyo en electrónica, cableado eléctrico e informática. Supervisión de las conexiones eléctricas y los sistemas informáticos del proyecto. |
| **Raquel Muñoz** | Seguimiento pedagógico del proyecto. |

---

## Alumnos

| Nombre | Rol principal |
|--------|--------------|
| **Simeon Crasiuc** | Firmware (Klipper / printer.cfg), electrónica, cableado, documentación GitHub. |
| **Pau Arias March** | Diseño y fabricación de piezas impresas en 3D, montaje mecánico. |
| **Daniel Roselló Díaz** | Montaje estructura, cableado, fotos del proceso. |
| **Sergio Tenorio Martínez** | Diseño CAD del marco, planos estructurales item, archivos 3D (STL/STEP pendientes de subir). |

---

## El instituto

El **Institut Jaume Huguet** es un centro de formación profesional de Valls (Tarragona) con más de 100 años de historia. Ofrece ciclos formativos de grado medio y superior en áreas como electricidad, electrónica, mecánica, informática y administración.

Este proyecto de impresora 3D de gran formato forma parte de un trabajo de clase con el objetivo de que la máquina **se quede en el instituto** para que pueda ser usada por futuros alumnos, replicada por otros centros, y sirva como plataforma de aprendizaje en fabricación digital avanzada.

---

## Versiones del repositorio

Este repositorio está disponible en tres idiomas:

| Idioma | Repositorio |
|--------|-------------|
| 🇪🇸 Español | **este repositorio** |
| 🏴󠁥󠁳󠁣󠁴󠁿 Català | [impresora-3d-1000x1000-ca](https://github.com/xXSimon91Xx/impresora-3d-1000x1000-ca) |
| 🇬🇧 English | [impresora-3d-1000x1000-en](https://github.com/xXSimon91Xx/impresora-3d-1000x1000-en) |
